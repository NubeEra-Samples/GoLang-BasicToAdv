...
// Function createNamesTable creates Names table in MySQL DB
// returns err if any
func createNamesTable(db *sql.DB) error {  
    query := `CREATE TABLE IF NOT EXISTS names (id int primary key auto_increment, name varchar(255), email varchar(255))`
    ctx, cancelfunc := context.WithTimeout(context.Background(), 5*time.Second)
    defer cancelfunc()
    res, err := db.ExecContext(ctx, query)
    if err != nil {
        log.Printf("Error %s when creating names table", err)
        return err
    }
...
func Index(w http.ResponseWriter, r *http.Request) {
	...
	db := dbConn()
	err := createNamesTable(db)
    if err != nil {
        log.Printf("Create names table failed with error %s", err)
        return
    }
